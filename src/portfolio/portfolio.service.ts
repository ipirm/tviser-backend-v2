...
export class PortfolioService extends TypeOrmCrudService<PortfolioEntity> {
  constructor(
    ...
    private readonly portfolio: I18nRepository<PortfolioEntity>,
    @InjectRepository(CategoryEntity)
    private readonly category: I18nRepository<CategoryEntity>,
    @InjectRepository(BrandEntity)
    private readonly brand: I18nRepository<BrandEntity>,
  ) {
    super(portfolio);
  }
  ...
  async createOneBase(
    ...
    if (dto.brandEntities) {
      const brandEntities = await this.brand.findByIds(
        dto.brandEntities
          .toString()
          .split(',')
